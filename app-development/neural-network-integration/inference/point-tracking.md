...
In this tutorial you will learn how to integrate your custom point tracking model into Supervisely by creating a simple serving app. As an example, we will use [PIPs](https://github.com/aharley/pips) repository, which already has the majority of the required functions developed.
...
* `predict`. The core implementation of a model inference. It takes list of images of `np.ndarray` type, inference settings and point to track as arguments, applies the model inference to the images and returns a list of predictions (both input point and predicted points are `sly.nn.PredictionPoint` objects).
...
```python
class PipsTracker(sly.nn.inference.PointTracking):
    def load_on_device(
        self,
        model_dir: str,
...
The following code creates the model according to config `supervisely/serve/model_settings.yaml`. Path to `.yaml` config is passed during initialization. Config in the form of a dictionary becomes available in `self.custom_inference_settings_dict` attribute. The `saverloader.load` function provided by the creator of the original repository loads the model state dict from `model_dir`. Also `load_on_device` will keep the model as a `self.model` and device as `self.device` for further use:
...
The core method for model inference. Here we are preparing images and getting an inference of the model. The function `sly_functions.run_model` is borrowed from the original repository. However, there are a few changes that can be made to improve quality: add padding before resizing, and make sure that predicted points are not outside of the bounds. Then we wrap model predictions into `sly.nn.PredictionPoint` and do some post-processing steps.
...
* `allowed_shapes` - shapes can be tracked with this model. Сonversion of figures to a set of points and vice versa is implemented in the base class, so keep it default.
